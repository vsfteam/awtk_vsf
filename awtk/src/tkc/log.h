...
#define log_debug(format, ...) \
  if (log_get_log_level() <= LOG_LEVEL_DEBUG) printf_impl(format, __VA_ARGS__)
#define log_info(format, ...) \
  if (log_get_log_level() <= LOG_LEVEL_INFO) printf_impl(format, __VA_ARGS__)
#define log_warn(format, ...) \
  if (log_get_log_level() <= LOG_LEVEL_WARN) printf_impl(format, __VA_ARGS__)
#define log_error(format, ...) \
  if (log_get_log_level() <= LOG_LEVEL_ERROR) printf_impl(format, __VA_ARGS__)
#endif
#elif defined(HAS_STDIO) || defined(AWTK_WEB)
#define log_debug(format, args...) \
  if (log_get_log_level() <= LOG_LEVEL_DEBUG) printf_impl(format, ##args)
#define log_info(format, args...) \
  if (log_get_log_level() <= LOG_LEVEL_INFO) printf_impl(format, ##args)
#define log_warn(format, args...) \
  if (log_get_log_level() <= LOG_LEVEL_WARN) printf_impl(format, ##args)
#define log_error(format, args...) \
  if (log_get_log_level() <= LOG_LEVEL_ERROR) printf_impl(format, ##args)
#else
#define log_debug(format, args...)
#define log_info(format, args...)
#define log_warn(format, args...)
